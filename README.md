...
## AS Numbers (Common to Both Labs)

| Device | AS | Role |
|---|---|---|
| PE1 / PE2 | 65001 | MPLS Provider |
| CE1 / CE2 | 65000 | Customer Edge |
| FG1 / PA-VM1 | 65100 | Site 1 Firewall |
| FG2 / PA-VM2 | 65200 | Site 2 Firewall |
| CF-POP1 | 65300 | SASE POP 1 |
| CF-POP2 | 65400 | SASE POP 2 |

---

## IPSec Tunnel Specifications

| Parameter | FortiGate (SD-WAN) | PA-VM (NGFW) |
|---|---|---|
| IKE Version | IKEv2 (default) | IKEv2 |
| Encryption | FortiOS 7.0.1 default* | AES-256-CBC |
| Hash | FortiOS 7.0.1 default* | SHA256 |
| DH Group | FortiOS 7.0.1 default* | Group 14 |
| PSK | fortinet123 | paloalto123 |
| MPLS-VPN Tunnel IP | 10.254.1.0/30 (FG1:.1, FG2:.2) | 10.254.1.0/30 (PA-VM1:.1, PA-VM2:.2) |
| SASE-VPN Tunnel IP | 10.255.2.0/30 (FG1:.1, FG2:.2) | 10.255.2.0/30 (PA-VM1:.1, PA-VM2:.2) |
| **Primary Path** | **SASE (priority-members 2 1)** | **MPLS (Local-Pref 200)** |
| **Secondary Path** | **MPLS** | **SASE (Local-Pref 100)** |
| Failover Trigger | Health Check SLA violation | BGP peer down detection |

\* FortiOS 7.0.1 does not support explicit `set proposal aes256-sha256` — removed and using platform defaults.

 **Note: Primary path is intentionally reversed between the two labs.**

 - FortiGate → SASE = Primary, MPLS = Secondary
 - PA-VM → MPLS = Primary, SASE = Secondary

 The FortiGate lab was designed this way to demonstrate brownout detection (SASE degradation → MPLS failover).

**【日本語サマリ】**<br>
プライマリパスは両ラボで意図的に逆に設定。<BR>
FortiGateはBrownout検証（SASE劣化→MPLSフェイルオーバー）のシナリオに合わせSASEをプライマリとし、PA-VMはMPLS優先（Local-Pref 200）の本番想定構成。
...
<img width="800" alt="image" src="https://github.com/user-attachments/assets/66c9b792-ac6e-4c4f-be92-074bd69420ee" />

<img width="600" alt="image" src="https://github.com/user-attachments/assets/38dad62d-2af6-46e8-a952-8e62e04c751a" />
...
<img width="700" alt="image" src="https://github.com/user-attachments/assets/8dd452ae-7b26-43be-b775-17a7a2edef4a" />
...
- [SASE-ZeroTrust](https://github.com/mikio-abe/SASE-ZeroTrust) - Security policy enforcement
- [Troubleshooting](https://github.com/mikio-abe/Troubleshooting) - Packet capture and visibility
