...
| FG1 | 65100 | Site 1 Firewall |
| FG2 | 65200 | Site 2 Firewall |
| CF-POP1 | 65300 | SASE POP 1 |
| CF-POP2 | 65400 | SASE POP 2 |

**【日本語サマリ】**<br>
PE（65001）、CE（65000）、Site1 FG（65100）、Site2 FG（65200）、SASE POP（65300/65400）。
...
| Encryption | FortiOS 7.0.1 default* |
| Hash | FortiOS 7.0.1 default* |
| DH Group | FortiOS 7.0.1 default* |
| PSK | fortinet123 |
| MPLS-VPN Tunnel IP | 10.254.1.0/30 (FG1: .1, FG2: .2) |
| SASE-VPN Tunnel IP | 10.255.2.0/30 (FG1: .1, FG2: .2) |
| **Primary Path** | **SASE (SD-WAN priority-members: 2 1)** |
| **Secondary Path** | **MPLS** |
| Failover Trigger | Health Check SLA violation |

\* FortiOS 7.0.1 does not support explicit `set proposal aes256-sha256` — removed and using platform defaults.

**【日本語サマリ】**<br>
IPSecトンネルを2本構成（MPLS-VPN / SASE-VPN）。<br>
SD-WANのpriority-membersでSASEをプライマリ、MPLSをセカンダリに設定。Health Check SLA違反でMPLSへ自動フェイルオーバー。<br>
FortiOS 7.0.1はproposal明示指定非対応のためデフォルト値を使用。
